...
- Review and merge #472 so `pnpm release:prepare` passes again; the Baseline Explorer needs a separate static deployment for the browser-data change. A local Playwright run needs `pnpm --filter @calavera/baseline-explorer exec playwright install chromium` once.
