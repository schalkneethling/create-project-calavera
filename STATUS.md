...
- CodeQL advanced setup (#495, branch `codeql-rust-paths-495`, pull request open, stacked on #484): GitHub's default CodeQL setup ran the Rust analysis on every pull request, about four minutes against about one minute for the actions and JavaScript analyses, and default setup offers neither path filtering nor per-language triggers. `.github/workflows/codeql.yml` replaces it: it analyzes `actions`, `javascript-typescript`, and `rust` with build mode `none` on pushes to `main`, on pull requests, and on a weekly schedule, and on a pull request a `changes` job diffs the merge base against the head and reports whether `apps/menu-bar/src-tauri/`, any `.rs` file, or a `Cargo.toml` or `Cargo.lock` changed. The CodeQL steps of the `rust` matrix entry are skipped when it did not. The condition sits on the steps rather than on the job because the `matrix` context is not available in `jobs.<job_id>.if`, so a job-level condition cannot be per matrix entry; a step-level condition keeps the job name `Analyze (rust)` present and stable on every event. Every action is pinned to a commit SHA and `pnpm workflow:check` passes. The project owner must disable default setup in Settings, Code security, Code scanning before GitHub accepts the advanced workflow's results; until then the upload step of every analysis fails. No standing test: tooling wiring is verified by hand at change time. No Changeset: workflows are not a published package.
- Maintenance outside Increment 1 (#472, branch `baseline-data-refresh-472`, pull request open): `pnpm release:prepare` failed because Dependabot bumped `baseline-browser-mapping` to 2.11.23 and the regenerated data (#471) left the snapshot cutoff, the hardcoded test pins, and the Changeset behind. `build:data` now derives the latest browser release from `getTimeline()`, prompts for a new non-future cutoff (or takes `--cutoff YYYY-MM-DD`), rewrites `scripts/snapshot.mjs` from a template, regenerates the data, and writes `.changeset/baseline-data-refresh.md` after confirmation (or with `--changeset`); `--check` fails with a distinct stale-cutoff message and never prompts. Tests derive the expected source versions from `package.json`. Data refreshed at cutoff 2026-09-15: widely available floor moved to Firefox 123 and Safari 17.4. Red tests, implementation, and docs landed separately from the regenerated data. Dependabot then bumped `web-features` to 3.38.0 on 2026-09-16 without regenerating, which the pull request CI in #474 caught; the data was regenerated at the same cutoff (475 features, floor unchanged) and the Changeset updated.
- Pull request CI (#446, branch `pr-ci-446`, pull request open): `.github/workflows/pull-request.yml` runs `pnpm check` (the same gate `publish.yml` runs before a release) on every pull request, on Node 24 with `pnpm install --frozen-lockfile --ignore-scripts` and the Explorer Chromium install, with `contents: read` and per-ref cancel-in-progress concurrency. Passes `pnpm workflow:check`. No Changeset: workflows are not a published package. Its first runs failed on `baseline-core`: the snapshot-cutoff test that #473 fixes, then the data check after Dependabot bumped `web-features` without regenerating (regenerated in #473). Both are the drift this gate exists to catch.
- Baseline data gate (#476, branch `baseline-check-gate-476`, pull request open, stacked on #474): `pnpm release:prepare` runs `pnpm baseline:check` (`build-data.mjs --check` in `baseline-core`) as its first gate after the frozen install, so a stale cutoff or stale data fails before lint, format, typecheck, and knip; on success `--check` prints one line with the cutoff and the source versions, so the release log carries the data state. The gate list is exported as `releaseGates` and its order is tested. No Changeset: the orchestrator is not published and `scripts/` in `baseline-core` is outside `files`.
- Repository linting folded into oxlint (#483, branch `oxlint-fold-483`, pull request open, stacked on #476): the repository linted itself with two linters and neither could fail a build. `oxlint.json` was never read, because oxlint discovers `.oxlintrc.json` only, and the `lint` script passed no `--deny-warnings`, so every rule reported at warning severity and exited 0; ESLint held the only enforced rules in an `eslint.config.js` reachable solely through `lint:js`, which nothing ran. The configuration is renamed to `.oxlintrc.json`, drops the `jsdoc` plugin that never ran, and carries `no-console` (with `clear` and `info` allowed) and `no-regex-spaces`; `lint` and `lint:fix` deny warnings. All seven ESLint findings are fixed rather than suppressed, except `logger.js`, which wraps `console` and carries a rule-scoped disable directive. ESLint, `@eslint/js`, `globals`, `lint:js`, and `eslint.config.js` are removed; ADR-0007 records the removal and defers jsdoc reactivation. No standing test guards the arrangement; ADR-0007 records the hand verification and why a test would only re-prove oxlint behaves as documented. No Changeset: root tooling only, and the `logger.js` change is a comment.
...
- ESLint removed from the repository's own tooling: `docs/adr/0007-remove-eslint-from-repository-tooling.md` (Accepted 2026-09-16). Repository tooling only; the catalog side was decided by ADR-0005. Reactivating jsdoc rules is deliberately left open.
...
- Review and merge #495 (CodeQL advanced setup), which sits at the top of the stack: #473, then #474, then #477, then #484, then #495, merged bottom-up. Before or immediately after it merges, turn off default setup in Settings, Code security, Code scanning; GitHub rejects results from an advanced workflow while default setup is enabled, so the workflow's upload step fails until the toggle is flipped.
...
- Merge #477 (Baseline data gate) after #474; the stack is #473, then #474, then #477, merged bottom-up.
- Merge the repository linting fold (issue #483) last in that stack. By pull request number the order is #473, then #474, then #477 (the Baseline data gate), then #484 (the #483 fold), merged bottom-up. It carries ADR-0007 and turns `pnpm quality` into a gate that can fail on a lint finding for the first time. Close Dependabot pull request #481 (`globals` 17.12.0) once it lands: the dependency is removed.
