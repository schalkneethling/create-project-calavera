...
A drift test, `scripts/lint-tooling.test.mjs`, holds the arrangement in place: the discovered configuration exists and parses, `oxlint.json` does not come back, the two rules keep their severity and options, `jsdoc` stays off, both lint scripts keep `--deny-warnings`, and no ESLint script, configuration file, or dependency returns to the root manifest. The root `test` script runs it.
